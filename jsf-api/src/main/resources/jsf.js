...
if (!((jsf && jsf.specversion && jsf.specversion >= 23000 ) &&
    ...
    jsf.push = (function(window) {
        ...
        var URI_PREFIX = "/javax.faces.push";
         ...
         */
        function ReconnectingWebsocket(url, channel, onopen, onmessage, onclose) {
            ...
            self.open = function() {
                ...
                socket.onmessage = function(event) {
                    onmessage(JSON.parse(event.data).data, channel, event);
                }
         ...
         * is valid and <code>jsf.push.close()</code> hasn't explicitly been called on the same channel.
         * @param port {number} The port of the web socket host. If undefined, then it defaults to window location port.
         * @param {string} uri The uri of the web socket representing the channel name and optional scope identifier. All
         * open web sockets on the same uri name will receive the same push notification from the server. The scope
         ...
         */
        self.init = function(port, uri, onopen, onmessage, onclose, autoconnect) {
            ...
            if (!sockets[channel]) {
                sockets[channel] = new ReconnectingWebsocket(getBaseURL(port) + uri, channel, resolveFunction(onopen), resolveFunction(onmessage), onclose);
            }
         ...
         * @return {string} Base URL of web socket endpoint (to suffix the channel name with).
         */
        function getBaseURL(port) {
            var host = port ? (window.location.hostname + ":" + port) : window.location.host;
            return URL_PROTOCOL + host + jsf.contextpath + URI_PREFIX + "/";
        }
